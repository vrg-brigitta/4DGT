...
def test_dynamic_replica_data_loader():
    ...
    for batch in data_loader:
        ...
        print(
            batch.keys())  # 'rgb_input', 'rays_t_un_input', 'cameras_input', 'rgb_output', 'rays_t_un_output', 'cameras_output', 'img_name_output', 'ratios_output', 'c2w_avg'
        ...
        assert batch['rgb_input'].shape[0] == batch_size, f"Batch size should be {batch_size}"
        assert batch['rays_t_un_input'].shape[0] == batch_size, f"Batch size should be {batch_size}"
        assert batch['cameras_input'].shape[0] == batch_size, f"Batch size should be {batch_size}"
        assert batch['rgb_output'].shape[0] == batch_size, f"Batch size should be {batch_size}"
        assert batch['rays_t_un_output'].shape[0] == batch_size, f"Batch size should be {batch_size}"
        assert batch['cameras_output'].shape[0] == batch_size, f"Batch size should be {batch_size}"
